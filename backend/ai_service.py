...
def build_stage_prompt(payload: AiTravelRecommendationRequest) -> str:
    visits = "\n".join(f"- {item.visit_date}: {item.branch_name}" for item in payload.branch_visits)
    legs = "\n".join(f"- {item.purpose}: {item.route}" for item in payload.transport_legs) or "Belum ada rute transport lokal."
    instructions = {
        "ticket": "Cari pilihan tiket pergi-pulang yang relevan. Bandingkan waktu, moda, dan estimasi biaya.",
        "hotel": "Cari hotel dekat cabang-cabang pada jadwal. Bandingkan jarak, estimasi per malam, dan kecocokan jadwal.",
        "local_transport": "Cari opsi transport paling tepat untuk setiap rute yang dipilih PIC. Berikan estimasi per leg.",
    }
...
{visits}
Preferensi PIC: {payload.preference}
Tiket yang dipilih pada tahap sebelumnya: {payload.selected_ticket or 'Belum dipilih'}
Hotel yang dipilih pada tahap sebelumnya: {payload.selected_hotel or 'Belum dipilih'}
Rute transport lokal yang wajib dianalisis:
{legs}

Jawab dalam Bahasa Indonesia. Beri 2-3 rekomendasi yang dapat dipilih, estimasi biaya,
alasan singkat, dan bagian 'Sumber web' berisi URL sumber yang ditemukan. Semua harga
harus diberi label estimasi dan tidak boleh diklaim sebagai ketersediaan atau harga live.
"""
